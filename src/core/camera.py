...
class Camera:
    ...
    def update(self, delta: float):
        dest = -(self.player_to_follow.position - self.screen_size / 2)
        # add mouse deviation
        ...
        self._camera_pos.x = pygame.math.lerp(self._camera_pos.x, dest.x,
                                              min(delta * abs(max(self.player_to_follow.velocity.x / 60, 5)), 1))
        self._camera_pos.y = pygame.math.lerp(self._camera_pos.y, dest.y,
                                              min(delta * abs(max(self.player_to_follow.velocity.y / 60, 5)), 1))
